// create a collection named iNotebook in thunder client in that create a folder Named notes inthat create a request called fetching all notes  in that add link "http://localhost:3001/api/note/fetchallnotes" and add auth-token of any one in thunder client =>header
// create a collection named iNotebook in thunder client in that create a folder Named notes inthat create a request called adding notes  in that add link "http://localhost:3001/api/note/addnote" and add auth-token of any one in thunder client =>header
...
router.get('/fetchallnotes',fetchuser,async(req,res)=>{//using fetchuser we get user data from token
    try {
        const notes=await Note.find({user:req.user.id})//fetching notes that created using a same user id
        res.send(notes);
    }
     catch (error) { // if error occur in try catch will run
        console.error(error.message);
...
router.post('/addnote',fetchuser,[//using fetchuser we get user data from token
    ...
    try {
        ...
        const note = new Note({
            title,description,tag,user:req.user.id//creating notes under same user id
        })
