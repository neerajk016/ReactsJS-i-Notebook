// create a collection named iNotebook in thunder client in that create a folder Named notes inthat create a request called updating note  in that add link "http://localhost:3001/api/note/updatenote/628f97e602759a9e49a1cb25" and add auth-token of any one in thunder client =>header
...
router.put('/updatenote/:id',fetchuser,async(req,res)=>{ // when you run fetch all notes youw will all notes in that for every note you will get an id paste id of your wish to edit
